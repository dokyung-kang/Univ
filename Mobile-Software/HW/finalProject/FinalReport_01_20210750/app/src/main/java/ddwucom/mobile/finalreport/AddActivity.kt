...
class AddActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addBinding.btnAddDiary.setOnClickListener{
            if (addBinding.etAddDiary.text.toString().equals("")){      //제목 필수 입력 확인
                ...
                val place = addBinding.etAddPlace.text.toString()
                val music = addBinding.etAddMusic.text.toString()
                val story = addBinding.etAddContent.text.toString()
                val newDto = DiaryDto(0, title, date, weather, place, music, story)      // 화면 값으로 dto 생성, id 는 임의의 값 0
    ...
    * DB추가 시 추가한 항목의 ID 값 반환, 추가 실패 시 -1 반환 */
    fun addDiary(newDto : DiaryDto) : Long  {
        ...
        newValues.put(DiaryDBHelper.COL_PLACE, newDto.place)
        newValues.put(DiaryDBHelper.COL_MUSIC, newDto.music)
