...
function moveToOriginalPosition() { // 선택된 이미지를 제자리로 이동시킴
	...
	for (var i = 0; i < cds.childNodes.length; i++){
		if(Number(cds.childNodes[i].id) > Number(imgElement.id))
			break;
	}
	
	// imgElement의 순위를 나타내는 <span>을 삭제
	imgElement.parentNode.removeChild(
		imgElement.previousSibling
	);

	// imgElement를 cds의 제 위치에 삽입 (주의: imgElement가 마지막 이미지인 경우)

	// imgElement에 대해 onclick 이벤트 핸들러 재설정 (//위로 올라갔으니 다시 아래로 내려가게 하는 이벤트 핸들러)

	// top5 이미지들의 순위 값을 재계산하여 변경 (//숫자값을 바꿔줌. 텍스트값만 바꾸면 됨 ex. 4 -> 3)
